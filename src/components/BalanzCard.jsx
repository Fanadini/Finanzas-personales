          ...
          return (
            <div key={label}>
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 text-left transition-colors ${expense ? 'active:bg-warm-50 cursor-pointer' : 'cursor-default'}`}
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <span className={`flex-shrink-0 transition-colors ${expense ? 'text-warm-400' : 'text-transparent'}`}>
                    <ChevronDown expanded={isExpanded} />
                  </span>
                  <span className="text-sm text-olive-700 truncate">{label}</span>
                </div>
                <span className={`text-sm font-semibold flex-shrink-0 ml-3 ${
                  amount == null ? 'text-warm-400'
                  : amount < 0 ? 'text-gold-500'
                  : 'text-olive-700'
                }`}>
              ...
              {isExpanded && (
                <div className="bg-warm-50 border-t border-warm-100">
                  {monthHistory.length > 0 ? (
                    <div className="divide-y divide-warm-100">
                      {monthHistory.map(d => (
                        <div key={d.label} className="flex items-center justify-between px-6 py-2">
                          <span className="text-xs text-warm-400">{d.label}</span>
                          <span className="text-xs font-medium text-olive-700">{formatARS(d.real)}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-xs text-warm-400 text-center py-3">Sin datos</p>
                  )}
                </div>
              )}
            </div>
